...
class ArrayOfTest extends FormTestCase
{
    ...
    public function test_generate()
    {
        ...
        $this->assertSame('!is_array($__tmp_44e18f0f3b2a419fae74cbbaef66f40e = $data->foo ?? null) ? null : (function ($value) {$transformer = $this->registry->getTransformerFactory()->create(\'Quatrevieux\\\Form\\\Embedded\\\ArrayItem\');$dataMapper = $this->registry->getDataMapperFactory()->create(\'Quatrevieux\\\Form\\\Embedded\\\ArrayItem\');$result = [];$errors = [];foreach ($value as $key => $item) {$transformationResult = $transformer->transformFromHttp((array) $item);if ($transformationResult->errors) {$errors[$key] = $transformationResult->errors;} else {$result[$key] = $dataMapper->toDataObject($transformationResult->values)->dto;}}if ($errors) {throw new \Quatrevieux\Form\Transformer\TransformerException(\'Some elements are invalid\', $errors);}return $result;})($__tmp_44e18f0f3b2a419fae74cbbaef66f40e)', $transformer->generateTransformFromHttp($arrayOf, '$data->foo ?? null', $generator));
        $this->assertSame('!is_array($__tmp_44e18f0f3b2a419fae74cbbaef66f40e = $data->foo ?? null) ? null : (function ($value) {$transformer = $this->registry->getTransformerFactory()->create(\'Quatrevieux\\\Form\\\Embedded\\\ArrayItem\');$dataMapper = $this->registry->getDataMapperFactory()->create(\'Quatrevieux\\\Form\\\Embedded\\\ArrayItem\');$result = [];foreach ($value as $key => $item) {$result[$key] = $transformer->transformToHttp($dataMapper->toArray($item));}return $result;})($__tmp_44e18f0f3b2a419fae74cbbaef66f40e)', $transformer->generateTransformToHttp($arrayOf, '$data->foo ?? null', $generator));
    }
